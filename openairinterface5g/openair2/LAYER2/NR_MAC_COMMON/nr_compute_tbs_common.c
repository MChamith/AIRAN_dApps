                        ...
                        uint8_t Nl)
{
  ...
  if (Qm == 0 || R == 0 || nb_rb == 0 || Nl == 0) {
    LOG_E(NR_MAC, "Error in compute TBS with a NULL input: Qm=%d R=%d nb_rb=%d Nl=%d (nb_symb_sch=%d nb_dmrs_prb=%d nb_rb_oh=%d tb_scaling=%d), returning NULL TBS\n",
          Qm, R, nb_rb, Nl, nb_symb_sch, nb_dmrs_prb, nb_rb_oh, tb_scaling);
    return 0;
  }
