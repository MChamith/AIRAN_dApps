...
#include <openair2/E3AP/service_models/scheduler_sm/scheduler_sm.h>
#endif // E3_AGENT
...
// Check and apply scheduler policy changes from E3 SM
void nr_update_scheduler_policy(module_id_t module_idP)
{
  gNB_MAC_INST *mac = RC.nrmac[module_idP];
  
  NR_SCHED_ENSURE_LOCKED(&mac->sched_lock);
  
  // Check if policy change is ready
  pthread_mutex_lock(&e3_sm_scheduler_control->mutex);
  if (e3_sm_scheduler_control->policy_ready) {
    uint8_t requested_policy = e3_sm_scheduler_control->requested_policy;
    uint8_t current_policy = e3_sm_scheduler_control->current_policy;
    
    // Apply policy change if different
    if (requested_policy != current_policy) {
      const char *policy_name = (requested_policy == SCHEDULER_POLICY_ROUND_ROBIN) ? 
                                "Round Robin" : "Proportional Fair";
      LOG_I(NR_MAC, "[SCHEDULER] Policy switch: %d -> %d (%s)\n", 
            current_policy, requested_policy, policy_name);
      
      // Update policy function pointer
      if (requested_policy == SCHEDULER_POLICY_ROUND_ROBIN) {
        ...
        for (int i = 0; i < MAX_NUM_BEAM_PERIODS; i++) {
          mac->rr_last_scheduled_rnti[i] = 0;
        }
      } else {
        mac->dl_sched_policy = nr_dl_proportional_fair;
      }
      
      e3_sm_scheduler_control->current_policy = requested_policy;
    }
    
    // Clear policy ready flag
    e3_sm_scheduler_control->policy_ready = 0;
  }
  pthread_mutex_unlock(&e3_sm_scheduler_control->mutex);
}

// Collect per-UE scheduler statistics and send to E3 SM
void nr_collect_scheduler_stats(module_id_t module_idP, 
                                frame_t frame, 
                                slot_t slot,
                                const nr_dl_candidate_t *candidates,
                                const nr_dl_alloc_t *allocs,
                                int n_candidates)
{
  gNB_MAC_INST *mac = RC.nrmac[module_idP];
  ...
  e3_sm_scheduler_control->sampling_counter++;
  
  // Check if it's time to collect statistics
  if (e3_sm_scheduler_control->sampling_counter > e3_sm_scheduler_control->sampling_threshold) {
    pthread_mutex_lock(&e3_sm_scheduler_control->mutex);
    
    // Populate per-UE statistics from candidates and allocations
    uint8_t num_ues = 0;
    for (int i = 0; i < n_candidates && num_ues < MAX_UE_SCHEDULER_STATS; i++) {
      const nr_dl_candidate_t *cand = &candidates[i];
      const nr_dl_alloc_t *alloc = &allocs[i];
      
      if (!cand->UE) continue;
      
      ue_scheduler_stats_t *stat = &e3_sm_scheduler_control->ue_stats[num_ues];
      
      // Basic UE info
      stat->rnti = cand->UE->rnti;
      
      // Throughput (from UE's average throughput in bps)
      stat->avg_throughput = (uint32_t)(cand->avg_throughput);
      
      // BLER (scaled by 100: 0-10000 range, e.g., 1234 = 12.34%)
      stat->bler = (uint16_t)(cand->bler * 10000.0f);
      
      // Current MCS
      stat->current_mcs = cand->current_mcs;
      
      // Pending bytes in RLC buffers
      stat->pending_bytes = cand->pending_bytes;
      
      // Retransmission flag
      stat->is_retx = cand->is_retx;
      
      // RBs allocated (if scheduled)
      stat->rbs_allocated = alloc->scheduled ? alloc->rbSize : 0;
      
      // Beam index
      stat->beam_index = cand->beam_index;
      
      num_ues++;
    }
    
    // Store metadata
    e3_sm_scheduler_control->last_frame = frame;
    e3_sm_scheduler_control->last_slot = slot;
    e3_sm_scheduler_control->num_ues = num_ues;
    
    // Set ready flag for E3 SM to read
    e3_sm_scheduler_control->stats_ready = 1;
    ...
    LOG_D(NR_MAC, "[SCHEDULER] Statistics collected: %d UEs, frame=%d, slot=%d\n", 
          num_ues, frame, slot);
  }
}
#endif // E3_AGENT
                             ...
                             int n_candidates)
{
  ...
  for (int e = 0; e < n_new; e++) {
    ...
    allocs[i].min_rbSize = min_rbSize;
    n_scheduled++;
...
void nr_dl_round_robin(const nr_dl_sched_params_t *params,
                      const nr_dl_candidate_t *candidates,
                      nr_dl_alloc_t *allocs,
                      int n_candidates)
{
  const int min_rbSize = 5;
  const int min_prb_per_ue = 10;  // Equal allocation for Round Robin
  int n_scheduled = 0;
  ...
  for (int i = 0; i < n_candidates; i++) {
    ...
    allocs[i].rbSize = needed_rbs;
    allocs[i].mcs = candidates[i].current_mcs;
    allocs[i].min_rbSize = needed_rbs;
    n_scheduled++;
  ...
  /* Phase 3: New data UEs — Round-robin with equal PRB allocation */
  int max_ues_rr = params->n_rb_avail / min_prb_per_ue;
  max_ues_rr = min(max_ues_rr, params->max_num_ue - n_scheduled);
  ...
  // Build list of new-data candidates
  typedef struct { int idx; rnti_t rnti; } rr_entry_t;
  rr_entry_t entries[MAX_MOBILES_PER_GNB];
  int n_new = 0;
  ...
  for (int i = 0; i < n_candidates; i++) {
    ...
    entries[n_new].idx = i;
    entries[n_new].rnti = candidates[i].UE->rnti;
    n_new++;
  }
  ...
  // Find the starting position: next UE after rr_last_scheduled_rnti
  int start_idx = 0;
  
  // Find where the last scheduled RNTI is in the current candidate list
  rnti_t last_rnti = params->mac->rr_last_scheduled_rnti[params->beam_idx];

  
  if (last_rnti != 0) {
    for (int e = 0; e < n_new; e++) {
      if (entries[e].rnti == last_rnti) {
        start_idx = (e + 1) % n_new;  // Start from next UE
        break;
      }
    }
  } else {
  ...
  /* Round-robin allocation: equal PRBs per UE in circular order */
  int ues_scheduled_rr = 0;
  rnti_t last_allocated_rnti = params->mac->rr_last_scheduled_rnti[params->beam_idx];
  
  for (int offset = 0; offset < n_new && ues_scheduled_rr < max_ues_rr; offset++) {
    int e = (start_idx + offset) % n_new;  // Circular indexing
    int i = entries[e].idx;
       
    int rbStart, rbSize;
    // Find exactly min_prb_per_ue contiguous PRBs
    if (!find_first_available_rbs(params->vrb_map, candidates[i].bwp_start,
                                 candidates[i].bwp_size, params->slbitmap,
                                 min_prb_per_ue, &rbStart, &rbSize)) {
     
      continue;
    }

    if (rbSize < min_prb_per_ue) {
      continue;
    }

    // Allocate exactly min_prb_per_ue PRBs for fairness
    allocs[i].scheduled = true;
    allocs[i].rbStart = rbStart;
    allocs[i].rbSize = min_prb_per_ue;
    allocs[i].min_rbSize = min_prb_per_ue;  // Enforce equal allocation
    n_scheduled++;
    ues_scheduled_rr++;

    // Mark allocated RBs
    for (int rb = 0; rb < min_prb_per_ue; rb++)
      params->vrb_map[rbStart + rb + candidates[i].bwp_start] |= params->slbitmap;
    
    // Track last allocated RNTI for next round-robin iteration
    last_allocated_rnti = entries[e].rnti;
  }

  
  // Update the round-robin state in MAC instance
  if (ues_scheduled_rr > 0) {
    params->mac->rr_last_scheduled_rnti[params->beam_idx] = last_allocated_rnti;
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (candidates[i].is_retx) {
      ...
      allocs[i].rbSize = sched_pdsch.rbSize;  // Update to actual allocated RBs based on TBS        
...
#ifdef E3_AGENT
  // Collect per-UE statistics for E3 SM (with sampling)
  nr_collect_scheduler_stats(mac->Mod_id, frame, slot, candidates, allocs, n);
#endif
}
...
#ifdef E3_AGENT
  // Check and apply scheduler policy changes before scheduling
  nr_update_scheduler_policy(mac->Mod_id);
#endif
