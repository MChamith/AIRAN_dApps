...
typedef struct nr_dl_sched_params {
    struct gNB_MAC_INST_s *mac;    ///< pointer to MAC instance (for RR state, etc.)
    frame_t frame;
    slot_t slot;
...
typedef struct nr_dl_alloc {
    ...
    uint16_t min_rbSize;           ///< minimum RBs for nr_find_nb_rb (scheduler-specific)
...
typedef struct gNB_MAC_INST_s {
  ...
  /// Round-robin scheduling state: last scheduled RNTI per beam
  rnti_t rr_last_scheduled_rnti[MAX_NUM_BEAM_PERIODS];
