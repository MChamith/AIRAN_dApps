...
#ifdef E3_AGENT
    // In slot 8, exclude TDAs that use symbol 12 (reserved for spectrum sensing)
    if (slot == 8) {
      uint16_t symbol_12_mask = 1 << 12; // Bit mask for symbol 12
      if (tda_bitmap & symbol_12_mask) {
        continue; // Skip this TDA, it uses symbol 12
      }
    }
#endif
